...
import { Building2, Globe, Users, MapPin, Briefcase, DollarSign, Calendar, Link2 } from 'lucide-react'
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {empresa.vacantes_activas.map(v => (
              <div key={v.id_vacante} className="flex items-start justify-between gap-4 p-4 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                <div>
                  <Link to={`/busqueda/${v.id_vacante}`} className="font-medium text-gray-900 hover:text-primary-600">
                    {v.titulo_puesto}
                  </Link>
                ...
                <Link to={`/busqueda/${v.id_vacante}`} className="text-xs font-medium text-primary-600 hover:underline whitespace-nowrap">Ver →</Link>
              </div>
            ))}
          </div>
        </div>
