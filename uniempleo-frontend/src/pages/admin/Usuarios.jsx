...
import { UserX, Trash2, ExternalLink } from 'lucide-react'
...
const Usuarios = () => {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      const r = await adminService.usuarios(params)
  ...
  const eliminar = async (id) => {
    if (!confirm('¿Eliminar (anonimizar) esta cuenta? No se puede deshacer.')) return
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">
          Gestión de Usuarios <span className="text-gray-400 text-lg font-normal">({total})</span>
        </h1>
      </div>

      {/* Filtros */}
      <div className="card mb-6 flex flex-wrap gap-3">
        <select value={tipo} onChange={e => setTipo(e.target.value)} className="input-field w-auto">
        ...
        <select value={activo} onChange={e => setActivo(e.target.value)} className="input-field w-auto">
            ...
            <tbody>
              {usuarios.map(u => (
                <tr key={u.id_usuario} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600 text-xs">{u.correo_electronico}</td>
