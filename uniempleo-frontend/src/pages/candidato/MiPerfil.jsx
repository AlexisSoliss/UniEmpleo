...
import { Upload, Loader2, FileText, Plus, Trash2, Pencil, Camera, X } from 'lucide-react'
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50 mt-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Puesto *</label>
          <input {...register('puesto')} className="input-field text-sm" placeholder="Desarrollador Backend" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Empresa *</label>
          <input {...register('empresa')} className="input-field text-sm" placeholder="Empresa SA de CV" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Ubicación</label>
          <input {...register('ubicacion')} className="input-field text-sm" placeholder="Monterrey, NL" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Fecha inicio *</label>
          <input {...register('fecha_inicio')} type="date" className="input-field text-sm" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Fecha fin</label>
          <input {...register('fecha_fin')} type="date" className="input-field text-sm" /></div>
      ...
      <div><label className="block text-xs font-medium text-gray-700 mb-1">Descripción</label>
        <textarea {...register('descripcion')} rows={2} className="input-field text-sm" /></div>
...
const FormEdu = ({ onGuardar, onCancelar }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="border border-gray-200 rounded-lg p-4 space-y-3 bg-gray-50 mt-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Institución *</label>
          <input {...register('institucion')} className="input-field text-sm" placeholder="TecNM / ITESM..." /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Grado *</label>
          <input {...register('grado')} className="input-field text-sm" placeholder="Ingeniería..." /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Campo de estudio</label>
          <input {...register('campo_estudio')} className="input-field text-sm" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Promedio</label>
          <input {...register('promedio')} type="number" step="0.01" min="0" max="10" className="input-field text-sm" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Fecha inicio</label>
          <input {...register('fecha_inicio')} type="date" className="input-field text-sm" /></div>
        <div><label className="block text-xs font-medium text-gray-700 mb-1">Fecha fin</label>
          <input {...register('fecha_fin')} type="date" className="input-field text-sm" /></div>
...
const MiPerfil = () => {
  ...
  const onSubmit = async (data) => {
    try {
      const payload = {
        ...
        generacion_egreso:   data.generacion_egreso   || null,
  ...
  const quitarCV = async () => {
    if (!confirm('¿Quitar el CV activo? Podrás subir uno nuevo después.')) return
    try {
      await api.delete(`/perfil/cv`)
      toast.success('CV eliminado')
      cargar()
    } catch (err) { toast.error(err.response?.data?.message || 'Error al eliminar CV') }
  }

  // Subir foto de perfil usando URL (por simplicidad, no file upload)
  const handleFotoUrl = async (url) => {
    try {
      await perfilService.editar({ foto_url: url })
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {perfil && (
            <button onClick={() => navigate(`/perfil/${perfil.id_perfil}`)}
        ...
        {!editando && perfil ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <div className="w-16 h-16 rounded-full bg-primary-100 flex items-center justify-center overflow-hidden border-2 border-primary-200">
                  {perfil.foto_url
                    ? <img src={perfil.foto_url} alt="foto" className="w-full h-full object-cover" />
                    : <span className="text-2xl font-bold text-primary-600">{usuario?.nombre_completo?.[0]}</span>}
                </div>
              </div>
              <div>
                ...
                {perfil.titulo_profesional && <p className="text-xs text-gray-500 italic">{perfil.titulo_profesional}</p>}
              </div>
            </div>
            {perfil.sobre_mi && <p className="text-sm text-gray-700 border-t pt-3">{perfil.sobre_mi}</p>}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <input {...register('foto_url')} className="input-field" placeholder="https://ejemplo.com/mi-foto.jpg" />
              <p className="text-xs text-gray-400 mt-1">Pega la URL de una imagen de internet o sube la foto a un servicio como imgur.com</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Título profesional</label>
                <input {...register('titulo_profesional')} className="input-field" placeholder="Desarrollador Full Stack" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Carrera *</label>
                <input {...register('carrera')} className="input-field" placeholder="Ing. en Sistemas Computacionales" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Generación de egreso</label>
                <input {...register('generacion_egreso')} type="number" className="input-field" placeholder="2024" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Modalidad preferida</label>
              ...
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Promedio general</label>
                <input {...register('promedio_general')} type="number" step="0.01" min="0" max="10" className="input-field" placeholder="9.2" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">GitHub URL</label>
                <input {...register('github_url')} className="input-field" placeholder="https://github.com/usuario" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">Portafolio URL</label>
                <input {...register('portfolio_url')} className="input-field" placeholder="https://mipagina.dev" /></div>
              <div><label className="block text-sm font-medium text-gray-700 mb-1">LinkedIn URL</label>
                <input {...register('linkedin_url')} className="input-field" placeholder="https://linkedin.com/in/usuario" /></div>
            </div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Sobre mí</label>
              <textarea {...register('sobre_mi')} rows={3} className="input-field" placeholder="Cuéntanos sobre ti..." /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Certificaciones <span className="text-gray-400 text-xs">(separadas por coma)</span></label>
              <textarea {...register('certificaciones')} rows={2} className="input-field" placeholder="AWS Cloud Practitioner 2024, CCNA 2023" /></div>
      ...
      <div className="card">
        ...
        {experiencias.length === 0 && !mostrarFormExp && <p className="text-sm text-gray-400">No has agregado experiencia laboral.</p>}
        <div className="space-y-4 mt-3">
          {experiencias.map(e => (
            <div key={e.id_experiencia}>
              {expEditando?.id_experiencia === e.id_experiencia ? (
                <FormExp inicial={e} onGuardar={() => { setExpEditando(null); cargar() }} onCancelar={() => setExpEditando(null)} />
              ) : (
      ...
      <div className="card">
        ...
        {mostrarFormEdu && <FormEdu onGuardar={() => { setMostrarFormEdu(false); cargar() }} onCancelar={() => setMostrarFormEdu(false)} />}
        {educacion.length === 0 && !mostrarFormEdu && <p className="text-sm text-gray-400">No has agregado educación.</p>}
