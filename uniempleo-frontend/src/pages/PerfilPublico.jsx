...
import {
  MapPin, Globe, FileText, Briefcase, GraduationCap, Star,
  ChevronDown, ChevronUp, Link2, Code2, ArrowLeft
} from 'lucide-react'
...
// Parsea de forma segura cualquier campo JSON del backend
// Siempre devuelve un array, nunca undefined ni null
...
const SeccionCard = ({ titulo, icono: Icon, children }) => {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      {abierto && (
        <div className="px-6 pb-5 border-t border-gray-100">{children}</div>
      )}
    </div>
  )
}
...
const PerfilPublico = () => {
  ...
  if (!perfil) return (
    <div className="text-center py-20">
      ...
      <button onClick={() => navigate(-1)} className="text-primary-600 hover:underline text-sm mt-2">
        ← Regresar
      </button>
    </div>
  )

  // Parseo seguro — NUNCA va a fallar con .map()
  const habilidades = parseArray(perfil.habilidades_tecnicas)
  const idiomas     = parseArray(perfil.idiomas)
  const areas       = parseArray(perfil.areas_interes)
  const experiencias = Array.isArray(perfil.experiencias) ? perfil.experiencias : []
  const educacion    = Array.isArray(perfil.educacion)    ? perfil.educacion    : []
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-4">
        ...
        <div className="px-6 pb-5">
          <div className="flex items-end justify-between -mt-10 mb-3">
            ...
            {perfil.cv_activo?.url_almacenamiento && (
              <a
                href={cvUrl(perfil.cv_activo.url_almacenamiento)}
                target="_blank"
                rel="noreferrer"
          ...
          {perfil.titulo_profesional && (
            <p className="text-gray-600 mt-0.5">{perfil.titulo_profesional}</p>
          )}
          <p className="text-sm text-gray-500 mt-1">
            {perfil.carrera}{perfil.generacion_egreso ? ` · Gen. ${perfil.generacion_egreso}` : ''}
          </p>
          {perfil.modalidad_preferida && (
            <p className="text-sm text-gray-400 flex items-center gap-1 mt-1">
              <MapPin size={13} />
              Modalidad preferida:
              <span className="capitalize ml-1">{perfil.modalidad_preferida}</span>
            </p>
          )}
            ...
            {habilidades.map((h, i) => (
              <span key={i}
                ...
                {typeof h === 'object' && h !== null
                  ? `${h.nombre || h.name || JSON.stringify(h)}`
                  : String(h)}
              </span>
            ))}
          </div>
      ...
      {/* ── Áreas de interés ── */}
      {areas.length > 0 && (
        <SeccionCard titulo="Áreas de interés" icono={Star}>
          <div className="flex flex-wrap gap-2 mt-4">
            {areas.map((a, i) => (
              <span key={i}
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    ...
                    {e.es_actual
                      ? 'Actualidad'
                      : e.fecha_fin
                        ? new Date(e.fecha_fin).toLocaleDateString('es-MX', { month: 'short', year: 'numeric' })
                        : ''}
                  </p>
                  {e.descripcion && (
                    <p className="text-sm text-gray-600 mt-2">{e.descripcion}</p>
                  )}
                </div>
              </div>
            ))}
              ...
              <div key={e.id_educacion} className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700">
                    {e.grado}{e.campo_estudio ? ` · ${e.campo_estudio}` : ''}
                  </p>
                  ...
                  {e.promedio && (
                    <p className="text-xs text-gray-500 mt-0.5">Promedio: {e.promedio}</p>
                  )}
                </div>
              </div>
            ))}
